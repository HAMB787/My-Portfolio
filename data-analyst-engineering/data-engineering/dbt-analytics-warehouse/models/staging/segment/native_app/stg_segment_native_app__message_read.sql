...
{% set ids        = ['anonymous_id', 'context_device_id', 'context_instance_id', 'id', 'user_id', 'last_read_message_id', 'member_id'] %}
{% set strings    = ['context_app_build', 'context_app_name', 'context_app_namespace', 'context_app_version', 'context_device_manufacturer', 'context_device_model', 'context_device_name', 'context_device_type', 'context_ip', 'context_library_name', 'context_library_version', 'context_locale', 'context_os_name', 'context_os_version', 'context_timezone', 'event', 'event_text'] %}
{% set integers   = ['context_screen_height', 'context_screen_width'] %}
{% set floats     = ['context_screen_density'] %}
{% set booleans   = ['context_network_cellular', 'context_network_wifi'] %}
{% set arrays     = [] %}
{% set timestamps = ['loaded_at', 'original_timestamp', 'received_at', 'sent_at', 'timestamp', 'uuid_ts'] %}

with
base_source as (select * from {{ source('segment_native_app', 'message_read') }}),
