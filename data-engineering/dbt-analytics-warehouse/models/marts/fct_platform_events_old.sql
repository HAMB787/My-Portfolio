...
feature_events AS (SELECT * FROM {{ ref('stg_pubsub__events_historical') }}),
feature_events_old AS (SELECT * FROM {{ ref('stg_pubsub__events_historical_old_events') }}),
click_events_mapping AS (SELECT * FROM {{ ref('stg_lookup_tables__click_events_mapping') }}),

feature_events_filtered AS (

    SELECT *
    FROM feature_events
    WHERE event_name NOT IN (SELECT DISTINCT event_name FROM feature_events_old)
...
-- ================================================== all the custom event logic is going here ================================
    ...
    CASE
        WHEN E.event_name = 'scheduled_session'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.meeting_type') = 'matching'
        ...
        WHEN E.event_name = 'canceled_meeting'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.meeting_type') = 'matching'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.rescheduled') = 'True'
        ...
        WHEN E.event_name = 'canceled_meeting'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.meeting_type') = 'matching'
            AND NOT JSON_EXTRACT_SCALAR(E.payload, '$.rescheduled') = 'True'
        ...
        WHEN E.event_name = 'onboarding_session_show'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.meeting_type') = 'matching'
        ...
        WHEN E.event_name = 'onboarding_session_noshow'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.meeting_type') = 'matching'
        ...
        WHEN E.event_name = 'customer_subscription_created'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.plan_category') = 'matching'
        ...
        WHEN E.event_name = 'typeform_question_passed'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.typeformquestionid') = '87965444-1f39-4df5-9c4b-54bc89dd619b'
            THEN 'typeform_question_passed_first_level' -- first important question checkpoint passed
        WHEN E.event_name = 'typeform_question_passed'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.typeformquestionid') = 'c390183a-e025-46b7-90c9-ddfadb66f809'
            THEN 'typeform_question_passed_second_level'
        WHEN E.event_name = 'typeform_question_passed' -- second important question checkpoint passed
            AND JSON_EXTRACT_SCALAR(E.payload, '$.typeformquestionid') = 'a4c8ad7e-a422-4234-8178-a4f5d8823375'
            THEN 'typeform_question_passed_third_level' -- third important question checkpoint passed
        WHEN E.event_name = 'page_view'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.url') LIKE '%matching=true%'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.path') = '/plans'
            THEN 'planupgrade_matchingsession_viewed'
        WHEN E.event_name = 'page_view'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.url') LIKE '%matching=true%'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.url') NOT LIKE '%mode=payment%'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.url') LIKE '%checkout-complete%'
            THEN 'planupgrade_matchingsession_purchased'
        WHEN E.event_name = 'member_webapp_clicked'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.click_id') IN (
            ...
            THEN 'planupgrade_matchingsession_viewed'                                              
        WHEN E.event_name = 'pricing_page_clicked'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.click_id') = 'stepper_continue_button'
            THEN 'pricing_page_clicked'                                         
        WHEN E.event_name = 'pricing_page_clicked'
            AND JSON_EXTRACT_SCALAR(E.payload, '$.click_id') <> 'stepper_continue_button'
            THEN 'pricing_page_clicked_general'                                         
        WHEN E.event_name = 'get_started_button_clicked' -- this is a case when events are appearing with different names in BQ and Amplitude
            THEN 'funnel_getstartedbutton_click'             
        WHEN E.event_name = 'assigned_primary_member' -- requested by product/task to do this renaming
            THEN 'member_partnerrequest_completed'             
        WHEN E.event_name = 'typeform_submitted'
            THEN 'questionnaire_complete'             
        WHEN E.event_name = 'Application Backgrounded' -- renaming
            THEN 'application_backgrounded'             
        WHEN E.event_name = 'Application Installed' -- renaming
            THEN 'application_installed'             
        WHEN E.event_name = 'Application Opened' -- renaming
            THEN 'application_opened'             
        WHEN E.event_name = 'matchingContinueClicked' -- renaming
            THEN 'matching_continue_clicked'             
        WHEN E.event_name = 'matchingPopupOpened' -- renaming
            THEN 'matching_popup_opened'             
        WHEN E.event_name = 'member_webapp_clicked' -- lookup logic of gw events
            THEN LOWER(COALESCE(EM.event_name, E.event_name))             
        WHEN E.event_name = 'member_nativeapp_clicked' -- lookup logic of gw events
            THEN LOWER(COALESCE(EM.event_name, E.event_name))             
        ELSE E.event_name
    END                                                                                 AS event_name,
...
--------========================================================================================================================
    ...
    E.user_email,
    LOWER(E.source_platform)                                                              AS source_platform,
    LOWER(E.source_version)                                                               AS source_version,
    LOWER(JSON_EXTRACT_SCALAR(E.payload, '$.context_user_agent'))                         AS device_user_agent,
    LOWER(E.device_type)                                                                  AS device_type,
    LOWER(E.device_os)                                                                    AS device_os,
    LOWER(E.device_version)                                                               AS device_version,
    LOWER(E.geo_country)                                                                  AS geo_country,
    LOWER(E.geo_state)                                                                    AS geo_state,
    LOWER(E.geo_iso_code)                                                                 AS geo_iso_code,
    LOWER(E.geo_ip)                                                                       AS geo_ip,
    LOWER(E.geo_locale)                                                                   AS geo_locale,
    LOWER(E.geo_timezone)                                                                 AS geo_timezone,
    COALESCE(
        JSON_EXTRACT_SCALAR(E.payload, '$.context_campaign_3butm_source'),
        JSON_EXTRACT_SCALAR(E.payload, '$.context_campaign_ial_utm_source'),
        JSON_EXTRACT_SCALAR(E.payload, '$.context_campaign_utm_source'),
        JSON_EXTRACT_SCALAR(E.payload, '$.customer_data_metadata_utm_source'),
        JSON_EXTRACT_SCALAR(E.payload, '$.customer_data_utm_source'),
        JSON_EXTRACT_SCALAR(E.payload, '$.member_utm_source'),
        JSON_EXTRACT_SCALAR(E.payload, '$.metadata_utm_source'),
        JSON_EXTRACT_SCALAR(E.payload, '$.utm_source'),
        JSON_EXTRACT_SCALAR(E.payload, '$.utm_params_utm_source'),
        JSON_EXTRACT_SCALAR(E.payload, '$.campaign_source')
    )                                                                                       AS utm_source,
    COALESCE(
        JSON_EXTRACT_SCALAR(E.payload, '$.context_campaign_3butm_medium'),
        JSON_EXTRACT_SCALAR(E.payload, '$.context_campaign_ial_utm_medium'),
        JSON_EXTRACT_SCALAR(E.payload, '$.context_campaign_utm_medium'),
        JSON_EXTRACT_SCALAR(E.payload, '$.customer_data_metadata_utm_medium'),
        JSON_EXTRACT_SCALAR(E.payload, '$.customer_data_utm_medium'),
        JSON_EXTRACT_SCALAR(E.payload, '$.member_utm_medium'),
        JSON_EXTRACT_SCALAR(E.payload, '$.metadata_utm_medium'),
        JSON_EXTRACT_SCALAR(E.payload, '$.utm_medium'),
        JSON_EXTRACT_SCALAR(E.payload, '$.utm_params_utm_medium'),
        JSON_EXTRACT_SCALAR(E.payload, '$.campaign_medium')
    )                                                                                       AS utm_medium,
    COALESCE(
        JSON_EXTRACT_SCALAR(E.payload, '$.context_campaign_3butm_campaign'),
        JSON_EXTRACT_SCALAR(E.payload, '$.context_campaign_ial_utm_campaign'),
        JSON_EXTRACT_SCALAR(E.payload, '$.context_campaign_utm_campaign'),
        JSON_EXTRACT_SCALAR(E.payload, '$.customer_data_metadata_utm_campaign'),
        JSON_EXTRACT_SCALAR(E.payload, '$.customer_data_utm_campaign'),
        JSON_EXTRACT_SCALAR(E.payload, '$.member_utm_campaign'),
        JSON_EXTRACT_SCALAR(E.payload, '$.metadata_utm_campaign'),
        JSON_EXTRACT_SCALAR(E.payload, '$.utm_campaign'),
        JSON_EXTRACT_SCALAR(E.payload, '$.utm_params_utm_campaign'),
        JSON_EXTRACT_SCALAR(E.payload, '$.campaign_name')
    )                                                                                       AS utm_campaign,
    E.payload,
    CAST(NULL AS STRING)                                                                    AS device_uuid
FROM feature_events_filtered E
LEFT JOIN click_events_mapping AS EM ON JSON_EXTRACT_SCALAR(E.payload, '$.click_id') = EM.click_id
...
final AS (
    ...
    SELECT 
        ...
        CAST(NULL AS STRING) AS crm_source,
        CAST(NULL AS STRING) AS crm_medium,
        CAST(NULL AS STRING) AS crm_campaign,
        CAST(NULL AS STRING) AS crm_content,        
        U.payload,
        U.device_uuid
    FROM
        unioned AS U
